...
class Pages_model extends CI_Model {
	...
	public function get_all($page_type) {
		...
		$kids = $this->db->select('parent_id as "id"')->where('page_type',$page_type)->where('parent_id is not null')->group_by('parent_id')->order_by('sort, title asc')->get($this->_table);
		$sql_results = array();
		if($query->num_rows() > 0)
		{
			if($kids->num_rows() > 0) {
				$parentIds=array();
				foreach($kids->result_array() as $k=>$v) {
					foreach($v as $k=>$id);
					$parentIds[] = $id;
				}
...
//			debug::dump($sql_results);
//			debug::dump($query->result_array());
			//debug::dump($kidArray);
//			debug::dump($kids->result_array());exit;
