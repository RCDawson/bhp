...
class Template {
    ...
    function initialize($config = array()) {
        ...
        if ($this->_theme_locations) {
            ...
            array_push($this->_theme_locations, APPPATH . 'modules/admin/');
        }
    ...
    /**
     *
     * Global Messaging - uses CI's session flashdata (returns assoc_array)
     *
     *
     *   Prototype from a controller
     *   $this->template->_message('Product successfully updated', 'success', 'admin/products');
     *
     */

    //  Storage container for the messages
    public $_messages = array();

    /**
     * Fill storage container then redirect if $url argument is not NULL
     * @param	string      $response       Message to be displayed in view
     * @param	string      $type           Optional value passed for CSS declarations
     * @param	string      $url            Optional URL for redirecting
     */
    public function _message($response, $type = 'default', $url = NULL) {
        $this->_messages[$type][] = $response;
        $this->_ci->session->set_flashdata('messages', $this->_messages);
        if($url) redirect($url);
    }

    /**
     * Output messages to view
     * @param    string    $wrapper    Optional HTML to wrap message.
     *
     */
